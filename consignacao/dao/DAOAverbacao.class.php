<?php
	class DAOAverbacao{
		...
		function __construct($numExt, $eC, $pesC, $serMat, $bC, $proC, $per, $sC, $dataC, $dataE, $numPar, $mont, $taxJur, $toRoot, $conex){
			include_once($toRoot."beans/Averbacao.class.php");
			include_once($toRoot."utils/ConectarMySQL.class.php");
			$this->averbacao = new Averbacao($numExt, $eC, $pesC, $serMat, $bC, $proC, $per, $sC, $dataC, $dataE, $numPar, $mont, $taxJur);
			$this->conexao = $conex;
		}
		
		public function cadastrar(){
			$sql = "INSERT INTO averbacoes (ave_numero_externo, emp_codigo, pes_codigo, ser_matricula,	ban_codigo,	pro_codigo,	par_periodo, sta_codigo, ave_data_criacao, ave_data_encerramento, ave_numero_parcelas, ave_montante, ave_taxa_juros) VALUES ('".$this->averbacao->getNumeroExterno()."', ".$this->averbacao->getEmpCodigo().", ".$this->averbacao->getPesCodigo().", '".$this->averbacao->getSerMatricula()."', '".$this->averbacao->getBanCodigo()."', ".$this->averbacao->getProCodigo().", '".$this->averbacao->getParPeriodo()."', ".$this->averbacao->getStaCodigo().", '".$this->averbacao->getDataCriacao()."', '".$this->averbacao->getDataEncerramento()."', ".$this->averbacao->getNumeroParcelas().", ".$this->averbacao->getMontante().", ".$this->averbacao->getTaxaJuros().")";
			echo($sql);
		...
		public function alterar($valRef){
			$sql = "UPDATE averbacoes SET ave_numero_externo='".$this->averbacao->getNumeroExterno()."', emp_codigo=".$this->averbacao->getEmpCodigo().", pes_codigo=".$this->averbacao->getPesCodigo().", ser_matricula='".$this->averbacao->getSerMatricula()."',	ban_codigo='".$this->averbacao->getBanCodigo()."',	pro_codigo=".$this->averbacao->getProCodigo().", par_periodo='".$this->averbacao->getParPeriodo()."', sta_codigo=".$this->averbacao->getStaCodigo().", ave_data_criacao='".$this->averbacao->getDataCriacao()."', ave_data_encerramento='".$this->averbacao->getDataEncerramento()."', ave_numero_parcelas=".$this->averbacao->getNumeroParcelas().", ave_montante=".$this->averbacao->getMontante().", ave_taxa_juros=".$this->averbacao->getTaxaJuros()." WHERE ave_numero_externo=".$valRef;
			if(!$this->conexao->executar($sql)){
