<?php
	class DAOParcela{
		...
		function __construct($nP, $aNE, $sC, $perPar, $val, $lin, $toRoot, $conex){
			include_once($toRoot."beans/Parcela.class.php");
			include_once($toRoot."utils/ConectarMySQL.class.php");
			$this->parcela = new Parcela($nP, $aNE, $sC, $perPar, $val, $lin);
			$this->conexao = $conex;
		}
		
		public function cadastrar(){
			$sql = "INSERT INTO parcelas (par_numero_parcela, ave_numero_externo, sta_codigo, par_periodo_parcela, par_valor, par_link) VALUES (".$this->parcela->getNumeroParcela().", '".$this->parcela->getAveNumeroExterno()."', ".$this->parcela->getStaCodigo().", '".$this->parcela->getPeriodoParcela()."', ".$this->parcela->getValor().", '".$this->parcela->getLink()."')";
		...
		public function alterar($valRef){
			$sql = "UPDATE parcelas SET par_numero_parcela = ".$this->parcela->getNumeroParcela().", ave_numero_externo='".$this->parcela->getAveNumeroExterno()."', sta_codigo = ".$this->parcela->getStaCodigo().", par_periodo_parcela='".$this->parcela->getPeriodoParcela()."', par_valor = ".$this->parcela->getValor().", par_link = ".$this->parcela->getLink()." WHERE par_numero_parcela=".$valRef;
			if(!$this->conexao->executar($sql)){
		...
		public function getParcela($valRefNP, $valRefNE){
			...
			$this->parcela->setLink($linha["par_link"]);
			return $this->parcela;
		}
